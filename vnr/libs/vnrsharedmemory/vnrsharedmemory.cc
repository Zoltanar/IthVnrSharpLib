...
{
  if (auto p = d_->constCell(i))
    return QString::fromLatin1(p->language);
  else
    return QString();
}
...
QString VnrSharedMemory::dataText(int i) const
{
  if (auto p = d_->constCell(i))
    if (p->textSize > 0 && p->textSize <= d_->textCapacity())
      return QString::fromUtf16(p->text, p->textSize);
  return QString();
}
...
void VnrSharedMemory::setDataText(int i, const QString &v)
{
  if (auto p = d_->cell(i)) {
    ...
    if (v.size() <= limit) {
      ...
      memcpy(p->text, v.utf16(), v.size());
    } else {
      QString w = v.left(limit);
      p->textSize = w.size();
      //w.toWCharArray(p->text); 
      memcpy(p->text, w.utf16(), w.size());
    }
  }
}
