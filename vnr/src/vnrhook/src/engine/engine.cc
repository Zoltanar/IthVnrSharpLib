...
bool InsertNewWillPlusHook()
{
  ...
  for (auto addr : MemDbg::SearchMemory(characteristicInstructions, sizeof(characteristicInstructions)))
  {
    //GROWL_DWORD(addr);
    HookParam hp = {};
    hp.address = addr + 3;
  ...
  */
  ...
  for (auto addr : MemDbg::SearchMemory(pattern, sizeof(pattern), PAGE_EXECUTE, module_base_, module_limit_))
  {
    HookParam hp = {};
    hp.address = addr;
    hp.type = USING_UNICODE;
