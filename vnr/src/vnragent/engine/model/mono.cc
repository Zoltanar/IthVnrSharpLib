...
namespace Private {
  ...
  bool before_mono_string(winhook::hook_stack *s)
  {
    ...
    if (isBadText((const wchar_t*)p->chars, p->length)
        //|| addresses_.find(p) != addresses_.end()
        || hashes_.find(Engine::hashWCharArray((const wchar_t*)p->chars, p->length)) != hashes_.end())
      return true;
    ...
    if (!newText.isEmpty() && newText != oldText) {
      ...
      newText.toWCharArray((wchar_t*)p->chars);
      //addresses_.insert(p);
      hashes_.insert(Engine::hashWCharArray((const wchar_t*)p->chars, p->length));
    }
    return true;
  }
